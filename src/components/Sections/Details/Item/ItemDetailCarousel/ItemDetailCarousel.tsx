...
const ItemDetailCarousel = ({ slidersData: imageList, width, height, smallGap, hasTag, tagColor, tagLabel }: Props) => {
  const id = useId();
  const modalId = `item-detail-carousel-images-modal-${id}`;
      ...
      <StyledWrapper $height={height} $smallGap={smallGap}>
        ...
        {imageList.length > 1 && (
          <StyledBoxList $smallGap={smallGap}>
            {imageList.slice(0, MAX_IMAGES).map((imageItem, index) => {
              let hasLabel = false;
              if (imageList.length > MAX_IMAGES && index === MAX_IMAGES - 1) {
                hasLabel = true;
              }
              return (
                <StyledImageItem
                  ...
                  {hasLabel && (
                    <StyledPlusLabel>
                      <StyledTypographyLabel>+ {imageList.length - MAX_IMAGES}</StyledTypographyLabel>
                    </StyledPlusLabel>
                  )}
                </StyledImageItem>
              );
